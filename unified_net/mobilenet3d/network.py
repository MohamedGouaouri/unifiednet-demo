...
from hyperbox.networks.utils import make_divisible
...
class Mobile3DNet(BaseNASNetwork):
    def __init__(
        ...
        self.input_channel = make_divisible(32 * width_mult, 8)
        first_cell_width = make_divisible(16 * width_mult, 8)
        for i in range(len(width_stages)):
            width_stages[i] = make_divisible(width_stages[i] * width_mult, 8)
        # first conv
        self.first_conv = ConvLayer(in_channels, self.input_channel, kernel_size=3, stride=1, use_bn=True, act_func='relu6', ops_order='weight_bn_act')

        # first block
        first_block = OPS['3x3_MBConv1'](self.input_channel, first_cell_width, 1)

        self.input_channel = first_cell_width
        blocks = [first_block]
        stage_cnt = 0
        for width, n_cell, s in zip(width_stages, n_cell_stages, stride_stages):
            for i in range(n_cell):
                if i == 0:
                    ...
                    stride = 1
                op_candidates = [
                    OPS['3x3_MBConv3'](self.input_channel, width, stride),
                    OPS['3x3_MBConv4'](self.input_channel, width, stride),
                    #  OPS['3x3_MBConv6'](input_channel, width, stride),
                     OPS['5x5_MBConv3'](self.input_channel, width, stride),
                    #  OPS['5x5_MBConv4'](input_channel, width, stride),
                     OPS['7x7_MBConv3'](self.input_channel, width, stride),
                    #  OPS['7x7_MBConv4'](input_channel, width, stride),
                ]
                if stride == 1 and self.input_channel == width:
                    # if it is not the first one
                    op_candidates += [
                        OPS['Zero'](self.input_channel, width, stride),
                        OPS['Identity'](self.input_channel, width, stride)
                    ]
                conv_op = OperationSpace(op_candidates, mask=self.mask, return_mask=True, key="s{}_c{}".format(stage_cnt, i))
                # shortcut
                if stride == 1 and self.input_channel == width:
                    # if not first cell
                    shortcut = IdentityLayer(self.input_channel, self.input_channel)
                else:
                    shortcut = None
        ...
        self.feature_mix_layer = ConvLayer(
            self.input_channel, last_channel, kernel_size=1,
    ...
    def reset_first_block(self, in_channels):
        self.first_conv = ConvLayer(
            in_channels, self.input_channel, kernel_size=3, stride=2,
