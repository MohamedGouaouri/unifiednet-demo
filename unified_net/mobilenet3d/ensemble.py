...
class SharePart3D(BaseNASNetwork):
    def __init__(
        ...
        # first conv
        first_conv = ConvLayer(in_channels, self.input_channel, kernel_size=3, stride=2, use_bn=True, act_func='relu6', ops_order='weight_bn_act')
        # first block
        first_block_conv = OPS['3x3_MBConv1'](self.input_channel, first_cell_width, 1)
        ...
        for width, n_cell, s in zip(width_stages, n_cell_real_stages, stride_stages):
            ...
            for i in range(n_cell):
                if i == 0:
                    ...
                    stride = 1
                calibrate_op = CalibrationLayer(self.input_channel, width, stride)
...
class BranchPart3D(BaseNASNetwork):
    def __init__(
        ...
        # feature mix layer
        self.last_channel = make_divisible(1280 * width_mult, 8) if width_mult > 1.0 else 1280
